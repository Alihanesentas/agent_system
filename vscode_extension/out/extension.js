...
function activate(context) {
    ...
    let disposableToggleMcp = vscode.commands.registerCommand('agentSystem.toggleMcpMode', async () => {
        const config = vscode.workspace.getConfiguration('agentSystem');
        const currentMcp = config.get('useMcpMode') || false;
    ...
    let disposableOpenDocs = vscode.commands.registerCommand('agentSystem.openDocs', () => {
        const panel = vscode.window.createWebviewPanel('agentDocs', '📖 Agent System Interactive Documentation', vscode.ViewColumn.One, {});
...
<body>
    ...
    let disposableTreeSim = vscode.commands.registerCommand('agentSystem.openTreeSimulation', () => {
        const panel = vscode.window.createWebviewPanel('agentTreeSim', '🌲 Agent Tree Topology & Real-Time Monitor', vscode.ViewColumn.One, {});
        panel.webview.html = `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    context.subscriptions.push(disposableRunTask, disposableConsensus, disposablePinout, disposableToggleMcp, disposableOpenDocs, disposableStats, disposableTreeSim, disposableAutoGoal, disposableLayers);
}
...
class AgentSidebarWebviewProvider {
    ...
    _getHtmlForWebview(webview) {
        const config = vscode.workspace.getConfiguration('agentSystem');
        const isMcp = config.get('useMcpMode') || false;
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
async function runTaskApi(promptInput, codeContext) {
    ...
    const useMcp = config.get('useMcpMode') || false;
